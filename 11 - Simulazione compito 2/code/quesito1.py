class Agent():
    ...
    def valid_state(self, state):
        return not  ( (state["wolf"] == state["goat"]    != state["boat"]) or\
                      (state["goat"] == state["cabbage"] != state["boat"]) )

    def next_states(self, state):
        for keys in [{'boat', k} for k,v in state.items() if v == state["boat"]]:
            next_state = {**state, **{k: 1-state[k] for k in keys}}
...
if False:
    class Agent():
        ...
        def valid_state(self, state):
            if (state["wolf"] == state["goat"] and state["boat"] != state["wolf"]) or \
                (state["goat"] == state["cabbage"] and state["boat"] != state["goat"]):
                return False
            else:
                return True
