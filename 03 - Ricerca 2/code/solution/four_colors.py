...
class DFSAgentOptim(DFSAgent):
    ...
    def forward_checking(self, state):
        return all(self.legal_values(state, node) > 0 for node in self.adjacency if node not in state) if self.fwc else True

    def MCV_sort(self, state):
        return min([n for n in self.adjacency if n not in state], key=lambda n: self.legal_values(state, n) if self.mrw else 0)

    def removed_neighbors_colors(self, state, node, color):
        neighbors = [ n for n in self.adjacency[node] if n not in state]
        return sum(self.legal_values(state, n) for n in neighbors) - sum(self.legal_values({**state, node: color}, n) for n in neighbors)

    def LCV_sort(self, state, node):
        return sorted(self.colors, key=lambda c: self.removed_neighbors_colors(state, node, c) if self.lcv else 0)
...
if __name__ == "__main__":
    ...
    a = DFSAgentOptim(colors, adjacency3)
